...
### The whole product, including the browser clients

`docker compose up` starts the API alone. To also get the operator portal and
the cardholder application, clone them as siblings of this repository and add
the overlay:
...
That adds five services to the three above: a short-lived job that creates the
portal and cardholder session databases with their own migration and runtime
roles, a migration container for each client, and the two applications.

| Application | URL | Sign in as |
| --- | --- | --- |
| API and Swagger | `http://localhost:5143` | bearer token from `/api/v1/auth/login` |
| Operator portal | `http://localhost:5179` | `company.admin@example.test` |
| Cardholder | `http://localhost:5180` | `recipient@example.test` |

Each client owns a small database used only for browser sessions, separate from
the platform's and from each other's. The browser never talks to the API
directly: the portal uses a same-origin BFF that keeps refresh tokens
server-side, which is why the API stays bearer-only and enables no broad CORS.

The POS till is deliberately not in this stack. Its device credential is
generated by the backend, returned once, and stored only as a hash, so there is
no way to hand a container a working secret without weakening the property that
makes the credential worth having. Register a till by hand when you want one.
