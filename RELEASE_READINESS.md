...
| Client correctness | Release builds and full automated suites for portal, cardholder, and POS | Source verified at the current metadata commits: portal 104, cardholder 177, and POS 97 tests passed locally. No hosted evidence exists: all three client repositories trigger CI only on `push` to `main`, so their `milestone/deployment-certified-rc` heads `ffda3a1`, `eb14295` and `9b61be7` have never been built by CI. Closing this needs a pull request per client repository, or the trigger widened as the backend already does | Maintainer and CI |
