...
| End-to-end transaction | Readiness for all five HTTP processes; runtime role check; forced RLS; recipient payment; POS confirmation; platform receipt; full refund | Run end to end against a live four-service stack on 2026-09-07 rather than only read: readiness on five processes, browser security headers on three, the runtime role and 26 forced-RLS tables, seeded organization and ownership, POS registration and device authentication, and a 1.00 USD sale fully refunded, emitting checksummed redacted evidence with no secret printed. Rehearsing the named-environment leg found that the acceptance recorder and two evidence scripts refused every release this project cuts, because each required an `-rc.` suffix; fixed with regression cases. The gate accepts named HTTPS endpoints, requires exact clean artifacts and pre-provisioned POS credentials, and correctly refuses the published demo password for a named environment. A real staging record remains | Maintainer |
...
| Deployment artifacts | Reproducible, versioned, immutable artifacts for all four applications; provenance and SBOM | Local clean packaging and executable probes are verified; coordinated run `32726035561` produced the four clean hosted bundles with embedded and sidecar SPDX 2.2 SBOMs from the hash-pinned generator; the intentional dispatch on `13c09e4` (run 32823299738) completed `Attest intentional release build`; a push-triggered run skips that job by design | Maintainer and CI |
...
| Release publication | Clean public histories, signed or protected tags, release notes, compatibility manifest, upgrade and rollback notes | Partly satisfied: all four repositories carry `v0.9.1` on `main` with release notes, an identical compatibility manifest, and a proven provenance and SBOM attestation path. A `v1.0.0` was cut and retracted the same day; see `VERSIONING.md`. Still outstanding: tag protection is a repository setting nobody has enabled, and rollback needs documented quota and phone-route compensating controls | Maintainer |
